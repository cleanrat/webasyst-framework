...
HELP;
    ...
    private function testRouting()
    {
        ...
        if (!empty($this->config['frontend'])) {
            ...
            || ($this->type == 'plugin' && empty($routing) && !empty($this->config['handlers']['routing']) && !$this->pluginClassMethodExists($this->config['handlers']['routing']))) {
                $result = false;
    ...
    {
        $file = wa()->getConfig()->getRootPath().'/wa-apps/'.$this->app_id.'/plugins/'.$this->extension_id.'/lib/'.$this->app_id.ucfirst($this->extension_id).'Plugin.class.php';
        if (is_readable($file)) {
            wa($this->app_id);
            include ($file);
